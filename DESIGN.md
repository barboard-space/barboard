...
>
> 维护约定：新增/改名组件时同步更新本文件。配套展示页：`styleguide.html`（精选「已确认标准」，逐项搬入）+ `styleguide-draft.html`（全量自动渲染底稿，供参考）。
...
> `:root` 颜色令牌分两组：**彩色 Chromatic**（accent/pink/violet/gold/board）+ **Mono 黑白灰**（bg/surface/border/text 系）。
...
| 部件 | class | 规格 |
|---|---|---|
| 容器 | `.nav__logo` | Bebas Neue **`--logo-fs-en`(26px, h-lg)** `!important`、weight 400、letter-spacing 0.02em、line-height 1、color `--clr-text`、flex `gap: --logo-gap-icon`(4px) |
| 图标 | `.nav__logo-img` | **26×26px**、opacity 0.92、`margin-top:-5px`（视觉对齐微调）|
| BOARD | `.nav__logo-board` | color **`--clr-board`**（彩色）、margin-left 1px |
...
| 部件 | class / 内联 | 规格 |
|---|---|---|
| logo | `.nav__logo.footer__logo` | 同版本 A + `margin-top:0!important`、`margin-bottom: --logo-gap-cjk`(10px) |
| 中文名 | 内联 `<p>` | **`--logo-fs-cjk`(12px, small)**、color `--clr-text`、margin-bottom 8px、文案「欧美流行音乐个人榜吧」 |
...
- **阶段一（本文件）**：组件清单 + 命名速查表 ✅
- **阶段二**：组件展示页——`styleguide-draft.html`（全量自动渲染底稿）✅ + `styleguide.html`（精选「已确认标准」，逐项精修确认后搬入，**建设中**）。
