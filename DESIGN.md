...
| tagline | `.footer__tagline` | **14px**(body-lg)、`--clr-text-2`、max-width 260、line-height 1.6、margin-top 24px；两行：始于2013年5月20日 / UNITED BY MUSIC |
...
>
> **⚠️ 三表术语（[CLAUDE.md #143]，全文档统一）**：「**计分板**」= Scoreboard 矩阵(`.bvr-mtx`，6.4)；「**结果概览**」= Results 表(`.bvr-tbl`，6.3)；「**12 分**」= 12 Points 表(`.bvr-12`，6.5)。勿混为「所有表格」。
...
- **0 分 / Tele 列（[CLAUDE.md #143]）**：结果概览所有列的 **0 照常显示**（与计分板小分不同）；**某场 tele 投票人数=0 时，Tele 列整列隐藏**（表头 `th-tele` + 各行单元格）。
...
- **排序（默认，[CLAUDE.md #143]）**：行**严格按结果概览名次**——正式曲 total↓（平局级联见 6.7），**混淆曲一律排其后、内部按总分降序**；**投票人列按其官方曲名次升序重排**（`idxOf`，评委在前观众在后）→ 自投格(`.self`)沿主对角线（取代旧的「行按评委列序」）。前 4 列表头可点（`wireMatrixSort`）：选送者=恢复默认序，Total/Jury/Tele 数值降序。
- **0 分 / Tele 列（[CLAUDE.md #143]）**：**仅每格小分(`.pt`)的 0 不显示**；Total/Jury/Tele 冻结列(`.tot/.sj/.st`)的 0 **照常显示**。**某场 tele 投票人数=0 时，整个 Tele 列隐藏**（`.st` 冻结列 + 表头；结果概览同步隐藏 Tele 列）。
- **细节**：自投格斜纹（`.self`，base + 45° hatch；混淆行无自投格）；12 分金色；汇总分 DM Sans 13px；`.bvr-mw` 用 `display:inline-block; max-width:100%` 收缩包裹（无右侧空白、超宽横滚）。**粘性「选送者」列 + `border-collapse:separate`**（单元格只画右/下边框，左/上由外框）——避免折叠边框被粘性列盖不住而**滚动漏光**；`.rcp` `z-index:2` + 不透明 `--clr-bg`。
...
- **多组（第三届起 A 小众 / B 中众；第五届起加 C 大众）**：同 `matches:[{match:'A',venue:'小众组'},…]`，`matchEng` 的 `MAP` 覆盖 `SF/GF/A/B/C/E`（`A→GROUP A`…`C→GROUP C`、`E→ENTERTAINMENT`，新增组别在此补）；各组独立计分排名。
...
- **名次（权威 = `scripts/recompute_bv_ranks.py`，[CLAUDE.md #144]）**：parse 产出的 rank 仅临时，**导入后必跑 recompute（SOP 第 5 步）**。正式曲 total↓，同分按 **Eurovision 平局级联**打破：① tele 总分↓ ② 给分人数(jury+tele)↓ ③ 12/10/8/7/6/5/4/3/2/1 分布↓ ④ running order(eid)↑。混淆曲并排名次 = 不低于其分的正式曲数+1（同分排正式后）。脚本幂等、`--write` 落盘，对全部届统一生效。
- **投票折算/惩罚（第五届 C 组）**：选送了歌但未投评委票者（昵称不在投票人列），其曲 `score=round(各票和×0.7)`；**Jury/Tele 显原始票（和≠score）、总分显折算后**，计分板 `note` 写明各人原始总分（`{m:昵称}` token）。识别：CSV 总分≈各票和×0.7。详见 CLAUDE.md #142。
- **混淆再投（第五届起）**：投混淆曲的票可由投票人等额再投一正式曲；原始矩阵已含再投票，正式曲分=列和直接读，无需特殊处理。
- **脏源 CSV 文本覆盖（第五届）**：源 CSV 标题被 Excel `#`/gbk `?` 损坏时，parser 用 `OVERRIDE`（按 CSV 行序整表覆盖 artist/song/language，用户核对版，含重音/feat/多语言空格分隔）。详见 CLAUDE.md #142。
