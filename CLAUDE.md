...
181. **⭐⭐⭐ 2015 年榜导入（成员列存"点数"而非"名次"的全新格式，新增 `POINTS_NOT_RANK_YEARS` 反推个人排名机制）**：源 `D:\Genius\BarChart\吧榜文件\年终榜\2013-2017贴吧年终榜\个人榜吧2015年吧友汇总榜.xlsx`，唯一 sheet `Sheet1`（openpyxl 报告的 `max_col` 是虚高的 Excel 极限值 16384，需先扫非空单元格找真实边界，真实为 27 列×1281 行）。
    ...
    - **封面**：199/200（1 首 `Lana Del Rey — Music To Watch Boys To` 持续遇 iTunes 限流，多次重试仍 403，接受为暂时性未命中、后续可再补跑）；个人 Top10 21 人全部满 10 条、covers 除 1 人 9/10 外均满。
    - **改任意 2015 相关文件后重跑顺序**：`annual_corrections.py`/`parse_annual_chart.py` 改动 → `python scripts/parse_annual_chart.py 2015 --top 200` → `python scripts/gen_member_pages.py`（本届无新成员，`BUILT_PAGES` 不变）。Hub 页 + 全部 9 个年份页 `YEARS` 数组已加回 2015。
