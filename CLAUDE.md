...
182. **⭐⭐⭐⭐ 2014 年榜导入（完全无表头 + 匿名成员机制）+ 修复影响 2015 数据的系统性 `_FEAT_RE` bug**：源 `D:\Genius\BarChart\吧榜文件\年终榜\2013-2017贴吧年终榜\个人榜吧2014年吧友汇总榜(1).xlsx`，唯一 sheet `Sheet1`（27 列，与 2015 一样 openpyxl 报的 `max_col` 需扫真实边界）。
    ...
    - **封面**：200/200（首轮 198/200，重试后满）。**详见 CLAUDE.md #183**（2013 沿用完全相同的 `ANON_MEMBER_COLS`/`POINTS_NOT_RANK_YEARS` 套路）。

183. **⭐⭐⭐⭐⭐ 2013 年榜导入（老式 .xls 二进制格式 + 复用 2014 的匿名机制）**：源 `D:\Genius\BarChart\吧榜文件\年终榜\2013-2017贴吧年终榜\13年新版吧榜 - 副本.xls`，唯一 sheet `Sheet1`（670 行×19 列）。
    ...
    - **列结构（有表头，但用户要求仍按匿名处理）**：`Rank/origin/Artist(s)/Title/<13 位成员各占 1 列，表头=部分可辨认网名/简称如 FAK·城城·小耳·dope>/SUM/COUNT()`。**与 2014 不同，本届源表其实有部分可辨认的表头**（`城城`→城妈、`小耳`→耳妈、`dope`→嘟妈等历史别名都能对上），但用户明确要求**按 2014 同规则处理**（不做身份归因），故仍归入 `ANON_MEMBER_COLS`（`range(4,17)`，13 列），表头文字只留作代码注释、不用于解析。`FIXED_COLS["2013"]={"rank":0,"artist":2,"song":3,"pts":17,"assist":18}`——**该年 SUM/COUNT() 两列现成就是点数/助攻数**，不需要 #181 的助攻数通用兜底。
    - **官方 10 榜合榜 vs 技术 13 列**：与 2014（21 vs 24）、2016（32 vs 34）同类不一致，`BOARD_COUNT["2013"]=10` 按用户提供数字展示。
    - **用户预判「最多榜冠」可能算不出可忽略——实际本届可以自动算出**（Katy Perry — Roar，2 榜冠），因为该年源表 SUM/COUNT 列质量足够、`POINTS_NOT_RANK_YEARS` 反推链路正常工作；`no1` 总和=13，与 13 个匿名列一一对应，一致性良好。
    ...
    - **封面**：200/200（首轮 195/200，重试后满）。
    - **改任意 2013/2014 相关文件后重跑顺序**：`annual_corrections.py`/`parse_annual_chart.py` 改动 → 分别 `python scripts/parse_annual_chart.py 2013 --top 200` / `2014 --top 200`（两年都不产出成员数据，**不需要**跑 `gen_member_pages.py`，除非改动影响到其它年份的 member-annual-index.json）。Hub 页 + 全部 11 个年份页 `YEARS` 数组已加回 2013/2014。
