...
184. **⭐ Barvision 2026 半决赛分组抽签器（进行中，v1 已建、待细化）**：为第 16 届办一场 Eurovision 式 SF1/SF2 分组抽签仪式（屏幕抽签 + OBS 采集，无实体演播室）。**目标**：用历史投票数据 + 用户给的派别把「可能互投」的成员摊到两场,其余靠公开种子随机;**只求"显著削弱已知互挺同场",不吹"科学分离"**（对外表述照此）。
    - **文件** `barvision/2026/draw/index.html`：自包含单页 HTML（内联样式/纯 JS/SHA-256），noindex、不进导航、**密码锁**（`var DRAW_PASS`,当前占位 `barvision2026`,上线前改）。**尚未 push**（等用户发话）。开发页,不受站点 CSS 影响。1280×720 基准 + JS 等比缩放(`fit()`),用户直播屏 2K(2560×1440)16:9 → 渲染正好 2×。
