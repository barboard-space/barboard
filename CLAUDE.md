...
### 待建页面（按优先级）
- **Barvision 历史成绩数据体系（进行中，见 #129–#157）**：**第 1–13 届已全部导入**（2019 五届 + 2020 七届 + 2023 第十三届「年度制」详情页/成员页/总成绩单全部完成，见 #157）。**下一步：导入第 14 届（Tonghua 2024）/ 第 15 届（Jinzhong 2025）**——同 2023 年度制（SF1/SF2/GF），渲染已通用，按 #157 末「导入 SOP」走（产契约 JSON + 补 `BV_THEME`/`BV_STRIPE` 年度配色 + 跑管线）；源数据在 `D:\Genius\Barvision\Barvision 2024/2025`，intro 备稿见 `data/barvision/edition-intros-2023-2025.md`。剩余：HOF 历届前三改版 + 全量数据核对
- `about.html` — 关于榜吧完整历史
