...
156. **第十三届（Qiqihar 2023）导入 — 2023+ 新格式（⚠️ 数据层已完成 commit，详情页/渲染待建）**：
    ...
    - **⚠️ 剩余构建（下一阶段，大；做成通用，2024/2025 复用）**：① `bv-results-render.js` 加**按届主题化**（年度配色 + hero 海报）；`matchEng` 补 `SF1→SEMI-FINAL 1`/`SF2→SEMI-FINAL 2`；`BV_SLOTS` 已含 `13`。② **计分板 jury/tele 拆两块**（同含 jury+tele 的场=GF；仅 jury 的场=单表，往届不变）。③ 新板块 **参赛名单(含 genre)+比赛规则**。④ **主题化 hero**（海报 `assets/images/barvision/2023/poster.png`+珊瑚红/墨绿）。⑤ `gen_member_pages` **SF→GF 收敛**（每人一条记录=`overall_rank`）。⑥ 薄壳 `barvision/2023/regular-13.html`(EDITION_SRC) + barvision.html BUILT_EDITIONS 加 ⅩⅢ + EDITIONS 卡(2023 logo) + 跑 recompute/number_anon/gen_member_pages/gen_bv_editions_index + 校验。12 分表由渲染器自动从 `votes.points` 生成。
