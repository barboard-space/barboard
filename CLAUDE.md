...
179. **⭐ 2016 年榜导入（2017 暂缺、正在修复中）+ 两个新机制（FIXED_COLS 按下标兜底、身份不明简称按用户指认逐个消歧）**：源 `D:\Genius\BarChart\吧榜文件\年终榜\2013-2017贴吧年终榜\2016吧单曲年榜汇总.xlsx`（该目录同时存放 2013–2017，但目前只有 2016 有完整可用汇总表；2017 用户正在另行修复，届时按同一套流程追加）。
    ...
    - **`archive/annual/2016/index.html` 的 `YEARS` 数组不含 2017**（因为 2017 数据暂缺），年份导航自然按现有年份两两相邻跳转（2016 的"下一年"直接跳到 2018）；2017 数据到位后，Hub 页 + 全部年份页的 `YEARS` 数组需要同步加回 2017。
