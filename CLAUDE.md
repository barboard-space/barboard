...
174. **⭐ 榜吧年终榜导入（2020+2021+2022+2023 已完成，进行中）**：把旧「吧年榜」(2013–2023 存档活动) 建成类 BBL 榜单页，逐年导入。
    ...
    - ~~下一步：制作 `/archive/annual/` 年榜总览 Hub 页~~ **已完成**（见 #175）。~~下一步：2020/2023 年榜导入~~ **已完成**（宽表两种变体：2020 名次在前、2023 名次在后；菜妈占位成员）。**剩余**：铺 2013–2019 其它年份（各年 Excel 结构可能不同，`SHEET`/`FULL_SHEET`/`MEMBER_SHEETS`/`ABBR_OVERRIDE`/`PAIR_COL_YEARS`/`PAIR_COL_RANK_SECOND`/`annual_corrections.py` 按年增补，Hub 页 + 2020/2021/2022/2023 四页的 `YEARS` 常量记得同步加新年份）、年榜页排行榜细化、后续分榜类型（专辑榜/艺人榜/冠单榜等，Hub 页已预留 section 结构）、2023 封面未命中项可再跑一轮补齐。
