...
149. **第八届（2020，A/B 两组）已导入**：`scripts/parse_bv_edition8.py`（两 CSV）→ **`data/barvision/barvision-2020/regular-08.json`**；薄壳 `barvision/2020/regular-08.html`；BUILT_EDITIONS 加 Ⅷ（VIII 卡本就有）。**仅 A/B 两组（无 C）**。歌曲列「艺人 - 歌名」合并需拆分、语种由 CSV 提供。混淆仅 8A 2 首。**零渲染/布局改动**。本届两个新点（均与用户确认）：
    - **⭐ 神妈 = 匿名大妈**（外号 神妈/隐妈/神隐妈 都归「匿名」id 0 unclaimed）：parse 的 `ALIASES` 加 `神妈/隐妈/神隐妈→匿名`，`resolve`/`mid_of` 特判 匿名→`{id:0,unclaimed:true}`、member_id=0。**与 ed3/4 不同：本届匿名有正式曲**（神妈选送 Deafheaven 8A#10、Peggy Gou 8B#3 + Foxing 混淆），归档进 `member/0.html`（unclaimed 页，表格列出正式+混淆，无统计卡/走势图）。**`gen_bv_editions_index.py` 已加：roster/成员变动跳过 `匿名`/`unclaimed`**（否则匿名正式曲会误入名册「首次加入」）。
