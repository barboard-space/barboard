...
122. **移动端改动自动 push**：每次涉及移动端的改动（`@media` 规则、响应式布局、touch 行为等）完成后，无需用户另行要求，直接 `git add → commit → git push origin main`。原因：用户无法通过本地 `file://` 预览移动端效果，必须通过 GitHub Pages 在真机上查看。非移动端改动仍按默认流程（用户确认后再 push）。
123. **移动端改动严格隔离**：凡说"移动端改动"，只能在 `@media (max-width: 768px)`（或更小断点）块内写规则，不得修改块外的任何全局 CSS 或默认值。需要跨父容器移动元素时，用 JS `matchMedia` 做 DOM move（同一元素，不复制），不改 HTML 默认结构。改动前必须确认：这行代码在桌面端是否会有任何视觉变化？有则重做。
124. **全站断点与设备对应关系**：`768px` 是手机与桌面的核心分界线——`@media (max-width: 768px)` = 手机移动端，`769px+` = 桌面端/Mac。`1024px` 仅处理平板宽度的局部调整（主要是 bbl.html 侧栏列宽）。`480px` 针对小屏手机（iPhone SE 等）。说"移动端改动"即指 `max-width: 768px` 块内；说"桌面端"即 769px 以上。
125. **`ev-req-item--plain` modifier**：`events.html` 专用修饰类，去掉 `.ev-req-item` 的边框、背景和 `::before` 圆点，保留字号/行高/`strong` 颜色。用于 VOTING（Jury/Tele/Approval 说明文本）和 RULEBOOK 卡片正文，实现"纯文字列表"效果，代码：`.ev-req-item--plain { background:none; border:none; border-radius:0; padding-left:0; } .ev-req-item--plain::before { display:none; }`。
126. **移动端跨父容器元素重定位**：用 JS `matchMedia` + DOM `insertBefore` 物理移动同一元素，而非 CSS 隐藏/显示两份副本（副本方案会导致 `id` 重复、事件绑定失效）。模式：`var mq = matchMedia('(max-width:768px)'); function place(){ if(mq.matches) layoutParent.insertBefore(el, layout); else sidebar.insertBefore(el, label); } place(); mq.addEventListener('change', place);`。已用于 bbl.html 搜索框在移动端移到日期行下方。
...
130. **移动端 chart-item 布局规格（已确认，首页 + bbl hero 预览区的 `.chart-stat`）**：`@media (max-width: 768px)` 内最终定稿值——`.chart-item { grid-template-columns: 36px 34px 1fr auto; gap: 8px; padding: 6px 10px 6px 6px }`（上/右/下/左，**右 10px 故 stats 不贴边**）；`.chart-cover { 34×34 }`（桌面 42）；`.chart-rank { font-size: 18px }`（桌面 22）；`.chart-rank-col gap 4px`、`.chart-change 10px`、`.chart-change--new/--re 9px` 均沿用全局默认；`.chart-stats { margin-left: 6px; column-gap: 4px; row-gap: 0; grid-template-columns: auto auto }`；`.chart-song__title { white-space: normal; overflow: visible }`（14px/600 沿用全局）、`.chart-song__artist { white-space: normal; margin-top: 0 }`（12px 沿用全局）；`.chart-stat__label/val { font-size: 10px; line-height: 1.1 }`。修改时以此为基准微调，不要回退到旧的三列布局或 `display: none`。注：bbl.html **完整榜单**是另一套 `.bbl-stat`，见 [#133]。
...
132. **`style.css?v=N` 固定版本号 = 缓存陷阱（重要）**：`index.html`/`bbl.html`/`bbl/hof.html` 引用 `style.css?v=N`（固定查询串）。改 `style.css` 后若不升 `N`，浏览器/GitHub Pages CDN 认为 URL 未变、继续用缓存旧 CSS，**真机上看不到任何改动**（这正是"调来调去真机没变化"的根因）。规则：**每次改动 `style.css` 并 push 前，必须把这 3 个文件的 `?v=N` 同步 +1**（当前 `v=3`）。stats 块右对齐已贴卡片右缘（距 ~1px），无法再右移；要让它视觉"更靠右"只能缩窄块宽（缩小 `.chart-stat__label` 字号，因 4 汉字标签是块宽主因）。其余页面引用无版本号的 `style.css`，靠 Pages 默认 `max-age=600` 10 分钟后自动 revalidate。
133. **bbl.html 有两套榜单 stats，勿混淆（重要）**：① **hero 预览区**用全局 `.chart-stats`/`.chart-stat`（定义于 `style.css`，label 左 val 右竖排），与首页同款；② **完整榜单 `#bblFullList`** 用 bbl.html 内联的 **`.bbl-item-stats`/`.bbl-stat`**（桌面端：每个 72px、label 上 val 下居中、`.bbl-stat + .bbl-stat` 用 `border-left` 当竖分隔线、3 项含「本周点数」）。改 `style.css` 的 `.chart-stat` **不影响完整榜单**。完整榜单移动端（bbl.html `@media` 块）已改为与首页同款紧凑样式：`.bbl-item-stats` 改 `grid: auto auto`，`.bbl-stat { display:contents }`（盒子消失故 border-left 分隔线自动去除），`.bbl-stat__label` 8px / `.bbl-stat__val` 7px / lh 0.6；`.bbl-stat:last-child{display:none}`（特异度 0,1,1 > display:contents）隐藏本周点数。另注：`.bbl-chart-list .chart-item`（0,2,0）覆盖全局移动端 `.chart-item`（0,1,0），故完整榜单移动端 rank/cover 仍是 44px/42px（非首页的 36px）。
