...
- `barvision/2026/odds/index.html` — **赔率预测板块（已完成，见 #190）**：整页数据驱动读 `data/barvision/odds/odds.json`；两级按钮切换赛事×盘口（绝不滚动）；两套高亮（模型预测卡片 紫 / 各家最优赔率 蓝，并列组阈值规则）+ 趋势 + html2canvas 下载高清图；引擎 `scripts/build_odds_json.py`（xlsx→json），入口在首页 season 卡 + 2026 hero。
...
190. **⭐ 赔率预测板块 `/barvision/2026/odds/`（本次新建，整页数据驱动 + Excel→JSON 引擎工作流）**：Eurovision World `/odds` 式赔率榜，**读 `/data/barvision/odds/odds.json` 渲染、不写死任何赔率**。文件：页面 `barvision/2026/odds/index.html`（薄壳 + 内联 `odds-*` 样式，**页面独有组件、不动 `style.css`、无 `?v=`**）；数据/引擎 `data/barvision/odds/{odds.json,odds_prev.json,barvision_2026_odds.xlsx}` + `scripts/build_odds_json.py`；截图库 `assets/vendor/html2canvas.min.js`（v1.4.1 MIT，本地 vendor——项目避免国内可能被墙的外部 CDN）。
    ...
    - **⭐ 下载高清图（html2canvas，关键坑）**：捕获 `.odds-board`（含标题/图例/免责声明，自带品牌、自解释），`scale=max(2,dpr×2)`、文件名 `barvision2026-{event}-{market}.png`。**html2canvas 1.4.1 不解析 `color-mix()`/`oklab`、且忽略 box-shadow**——故**被捕获的 `.odds-board` 内部一律用 hex/rgba 显式色 + border（勿 color-mix、勿靠 box-shadow 做视觉）**；hero/按钮等未捕获区可用 color-mix。已验证导出的 PNG 里紫/蓝两种高亮 tint 都正确栅格化。⚠️ **html2canvas 不按浏览器算法排 `table-layout:auto` 的表**——会把表排窄、列左移、**右侧列缺失/整体错位**（活体 DOM 完全对齐、仅导出错位，用户反馈的「截图错位」根因）。**修复 `pinTableWidths(board)`**：导出前读每列当前实际宽度、用 `table-layout:fixed`+逐列 `th.style.width` 钉死（像素级不变），html2canvas 即按此确定布局渲染，导出后 revert（只在导出瞬间生效、不动屏显 auto 布局）。已像素级复验：11 列 x 位全对齐、行内竖直 drift 0。
