...
### Fixed
- **导出图片修复**：① 桌面（Windows 的 `canShare({files})` 也为 true，曾误弹系统分享框）改为**仅手机/触屏用 Web Share、桌面一律直接下载**；② 分辨率提升 2x→**3x** 且让克隆 SVG 以 `W*SC` 像素**直接栅格化**（原先先 1x 再放大变糊）；③ 导出时按**固定逻辑宽度 1120** 重绘再截取（之后即刻恢复响应式），使**手机也产出与桌面一致的宽幅高清图**（3552×1302）。
